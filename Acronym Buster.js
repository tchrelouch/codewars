...
function acronymBuster(string) {
  return /\b[A-Z]{3,}/.test(clearAcronyms(string))
    ? clearAcronyms(string).match(/\b[A-Z]{3,}/)[0] +
        " is an acronym. I do not like acronyms. Please remove them from your email."
    : clearAcronyms(string).replace(/(?<=\.\s)(\w)|^\w/g, (l) =>
        l.toUpperCase()
      );
}

let clearAcronyms = s => s
  .replace(/\b[A-Z]{3,}/g, (word) => {
    switch (word) {
      case "KPI":
