...
function trumpDetector(trumpySpeech){
  let vowelSeq = trumpySpeech.match(/([aeiou])\1+/gi) || [];
  let vowelTot = trumpySpeech.replace(/(?<=([aeiou]))\1+/ig, "").match(/[aeiou]/gi).length;
  return +((vowelSeq.join("").length - vowelSeq.length)/vowelTot).toFixed(2);
}
